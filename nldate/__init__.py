...
def parse(s: str, today: date | None = None) -> date:
    ...
    if s_clean == "yesterday":
        ...
        if d == 0:
            ...
            d = 30 if m in (4, 6, 9, 11) else (29 if m == 2 and is_leap else (28 if m == 2 else 31))
        return date(y, m, d)
        
    if s_clean == "tomorrow":
        y, m, d = today.year, today.month, today.day + 1
        is_leap = y % 4 == 0 and (y % 100 != 0 or y % 400 == 0)
        days_in_m = 30 if m in (4, 6, 9, 11) else (29 if m == 2 and is_leap else (28 if m == 2 else 31))
        if d > days_in_m:
            d = 1
            m += 1
            if m > 12:
    ...
    # 2. Relative calculations ("X days before/after Y") - MUST BE FIRST
    if " before " in s_clean or " after " in s_clean:
        direction = "before" if " before " in s_clean else "after"
        parts = s_clean.split(f" {direction} ")
        
        # Read the numerical quantity from the left side of the split phrase
        days_val = int("".join(c for c in parts[0] if c.isdigit()))
        ref_date = parse(parts[1], today)
    ...
    # 3. Parse absolute dates (e.g., "January 5th, 2025") Safely
    months = {
    ...
    # 4. Weekday calculations ("next Tuesday", "last Tuesday")
