...
def parse(s: str, today: date | None = None) -> date:
    ...
    # 1. Handle ISO-formatted dates (e.g., '2025-12-04')
    # Matches exactly YYYY-MM-DD
    iso_parts = s_clean.split("-")
    if (
        len(iso_parts) == 3
        and len(iso_parts[0]) == 4
        and iso_parts[1].isdigit()
        and iso_parts[2].isdigit()
    ):
        try:
            return date(int(iso_parts[0]), int(iso_parts[1]), int(iso_parts[2]))
        except ValueError:
            pass
    ...
    if " before " in s_clean or " after " in s_clean:
        ...
        parts: list[str] = s_clean.split(f" {direction} ")

        days_val: int = int("".join(c for c in parts[0] if c.isdigit()))
        ref_date: date = parse(parts[1], today)
    ...
    # 4. Parse absolute text dates (e.g., "January 5th, 2025") Safely
    months: dict[str, int] = {
        "jan": 1,
        "feb": 2,
        "mar": 3,
